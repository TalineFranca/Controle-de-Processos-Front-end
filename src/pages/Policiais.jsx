...
import { Search, MapPin, Plus, CheckCircle2, Clock } from 'lucide-react'
...
export default function Policiais() {
  ...
  const params = { limite: 300, ativo: true }
  ...
  // Carrega processos pendentes para exibir status na tabela
  ...
  const localidades = unidadesData?.dados?.localidades || []
  const processos = processosData?.dados || []

  // Agrupa processos por policial para fácil lookup
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-0.5">{lista.length} policiais encontrados</p>
      </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        <select
          ...
          className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-pm-500/30 bg-white"
        >
                  ...
                  {pols.map((p) => {
                    ...
                    return (
                      <tr key={p._id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-xs text-gray-500">{p.unidade || '—'}</td>
                        <td className="px-4 py-3">
                          <button
                            onClick={() => setModalPolicial(p)}
